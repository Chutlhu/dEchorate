...
path_to_dataset = os.path.join(dataset_dir, 'rir_matrix.npy')
path_to_annotation = os.path.join(dataset_dir, 'manual_annotation.csv')
path_to_delay = os.path.join(dataset_dir, 'database_delay.csv')
path_to_echo_annotation = os.path.join(dataset_dir, 'echo_annotation.pkl')
...
ax2 = fig.add_subplot(222, xlim=(20, 2000),
                      ylim=(-0.05, 1.05), sharex=ax1, sharey=ax1)
ax3 = fig.add_subplot(223, xlim=(20, 2000),
                      ylim=(-0.05, 1.05), sharex=ax1, sharey=ax1)
...
# Load RIRs and annotation
all_rirs = np.load(path_to_dataset)

toa_note = load_from_pickle(path_to_echo_annotation)
L, I, J, D = all_rirs.shape
K, I, J, D = toa_note['toa'].shape

taus_list = [r'%d $\tau_{%s}^{%d}$' % (
    k, toa_note['wall'][k, 0, 0, 0], toa_note['order'][k, 0, 0, 0]) for k in range(K)]
...
def plot_peak_annotation(peaks, rir, ax):
    ...
    for p, peak in enumerate(peaks):
        ax.annotate("%d - %1.2f" %
                    (p, np.round(peak, 2)), [peak, rir[int(peak)]])
...
initial_validity = [True if d == 1 else False for d,
...
class Callbacks():
    ...
    def set_extreme_dp_deconvolution(self, x, i):
        ...
        self._update()
        self.axes[0].fill_between(np.arange(
            self.dp_extreme[0], self.dp_extreme[1]), -0.2, 1.2, color='C8', alpha=0.2)
        self.axes[1].fill_between(np.arange(
            self.dp_extreme[0], self.dp_extreme[1]), -0.2, 1.2, color='C8', alpha=0.2)
        self.axes[2].fill_between(np.arange(
            self.dp_extreme[0], self.dp_extreme[1]), -0.2, 1.2, color='C8', alpha=0.2)
    ...
    def peak_peaking(self, event):
        ...
        peaks = pk.indexes(
            cb, thres=self.peak['threshold'], min_dist=self.peak['distance'])
        interpolated_peaks = pk.interpolate(
            np.arange(0, len(cb)), cb, ind=peaks, width=3)
    ...
    def dp_deconv(self, event):
        ...
        for d in range(D):
            ...
            dp_deconv = np.real(np.fft.ifft(
                np.fft.fft(rir, L) / np.fft.fft(dp, L)))[:L-p]
    ...
    def _update(self):
        ...
        plot_rirs_and_note(self.curr_rirs, self.toa_note, self.i, self.j,
                           self.idx, self.axes[0], self.visibility, self.dp_extreme)
        plot_rirs_and_note(self.curr_rirs_clean, self.toa_note, self.i,
                           self.j, self.idx, self.axes[1], self.visibility, self.dp_extreme)

        self.feature = plot_rirs_and_note_merged(
            self.curr_rirs_clean, self.toa_note, self.i, self.j, self.idx, self.axes[2])

        plot_staked_rirs(self.all_rirs, self.toa_note, self.i,
                         self.j, self.idx, self.axes[3])

        if not self.interpolated_peaks is None:
            plot_peak_annotation(self.interpolated_peaks,
                                 self.feature, self.axes[2])
...
cb = Callbacks(cp(toa_note), all_rirs, cp(all_rirs),
               datasets, initial_validity, fig, axes=axes)

check.on_clicked(cb.set_visibility)

dp_box1.on_submit(
    lambda string: cb.set_extreme_dp_deconvolution(int(string), 0))
dp_box2.on_submit(
    lambda string: cb.set_extreme_dp_deconvolution(int(string), 1))
...
txt_boxes_pck_dst.on_submit(
    lambda val_str: cb.set_peak_dict('distance', float(val_str)))
txt_boxes_pck_thr.on_submit(
    lambda val_str: cb.set_peak_dict('threshold', float(val_str)))
txt_boxes_pck_num.on_submit(
    lambda val_str: cb.set_peak_dict('num_peaks', int(val_str)))
